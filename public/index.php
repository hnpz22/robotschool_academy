...
// Cursos publicados &mdash; cada curso es &uacute;nico, sus sedes vienen de los grupos
$cursos_raw = $pdo->query("
    SELECT id AS curso_id, nombre, imagen, introduccion, objetivos,
           edad_min, edad_max, valor, tipo_valor, cupo_maximo, orden
    FROM cursos WHERE publicado = 1 ORDER BY orden ASC, nombre ASC
");
$cursos_raw = $cursos_raw ? $cursos_raw->fetchAll() : [];

$cursos_agrupados = [];
foreach ($cursos_raw as $c) {
    $stSedes = $pdo->prepare("
        SELECT DISTINCT s.id AS sede_id, s.nombre AS sede_nombre, s.ciudad AS sede_ciudad
        FROM grupos g JOIN sedes s ON s.id = g.sede_id
        WHERE g.curso_id = ? AND g.activo = 1 ORDER BY s.nombre
    ");
    $stSedes->execute([$c['curso_id']]);
    $sedes_curso = $stSedes->fetchAll();

    $sedes = [];
    foreach ($sedes_curso as $s) {
        $stg = $pdo->prepare("
            SELECT id, dia_semana, hora_inicio, hora_fin, modalidad, cupo_real,
                (cupo_real - COALESCE(
                    (SELECT COUNT(*) FROM matriculas m WHERE m.grupo_id=g.id AND m.estado='activa')
                ,0)) AS disponibles
            FROM grupos g WHERE curso_id=? AND sede_id=? AND activo=1
            ORDER BY dia_semana, hora_inicio
        ");
        $stg->execute([$c['curso_id'], $s['sede_id']]);
        $grupos = $stg->fetchAll();
        $sedes[] = [
            'curso_id'    => $c['curso_id'],
            'sede_id'     => $s['sede_id'],
...
$cursos_filtrados = array_filter($cursos_agrupados, function($c) use ($filtro_sede, $filtro_edad) {
    if ($filtro_sede) {
        $tiene = false;
        foreach ($c['sedes'] as $s) { if ($s['sede_id'] == $filtro_sede) { $tiene = true; break; } }
        if (!$tiene) return false;
    }
    if ($filtro_edad) {
        [$emin,$emax] = explode('-', $filtro_edad);
        if (($c['edad_min'] ?? 0) > (int)$emax || ($c['edad_max'] ?? 99) < (int)$emin) return false;
    }
    return true;
});
?>
    ...
    <?php else: ?>
      <div class="row g-4 stagger">
        <?php foreach ($cursos_filtrados as $c):
          $n_sedes    = count($c['sedes']);
          $total_cupos = array_sum(array_column($c['sedes'], 'cupos'));
        ?>
        <div class="col-sm-6 col-lg-4">
          <div class="curso-pub-card">
            ...
            <div class="curso-pub-body">
              <div class="mb-2" style="display:flex;align-items:center;gap:.4rem;flex-wrap:wrap;">
                <?php if ($c['edad_min'] && $c['edad_max']): ?>
                  <span class="age-badge"><?= $c['edad_min'] ?>&ndash;<?= $c['edad_max'] ?> a&ntilde;os</span>
                <?php endif; ?>
                <span style="font-size:.67rem;font-weight:800;padding:.2rem .6rem;border-radius:20px;background:rgba(29,169,154,.1);color:#0F6E56;">
                  <i class="bi bi-geo-alt-fill"></i>
                  <?= $n_sedes > 1 ? $n_sedes.' sedes disponibles' : h($c['sedes'][0]['sede_nombre']) ?>
                </span>
              </div>
              ...
              <?php if ($c['introduccion']): ?>
                <p style="font-size:.84rem;color:var(--muted);line-height:1.65;margin-bottom:.8rem;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;"><?= h($c['introduccion']) ?></p>
              <?php endif; ?>
              <?php if ($c['valor'] > 0): ?>
                <div style="font-size:.8rem;font-weight:800;color:var(--blue);margin-bottom:.8rem;">
              ...
              <?php endif; ?>
              <div style="display:flex;align-items:center;gap:.4rem;margin-bottom:.9rem;font-size:.78rem;font-weight:700;">
                <span style="width:8px;height:8px;border-radius:50%;background:<?= $total_cupos>0?'#16a34a':'var(--orange)' ?>;flex-shrink:0;"></span>
                <span style="color:<?= $total_cupos>0?'#16a34a':'var(--orange)' ?>;">
                  <?= $total_cupos>0 ? $total_cupos.' cupos disponibles' : 'Sin cupos &mdash; Lista de espera' ?>
                </span>
              </div>
              <button onclick="verCurso('<?= addslashes(h($c['nombre'])) ?>')"
                      class="btn-inscribir" style="width:100%;border:none;cursor:pointer;margin-top:auto;">
                <i class="bi bi-eye-fill"></i> Ver sedes, horarios e inscripci&oacute;n
              </button>
            </div>
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;">
          ...
          <div>
            ...
            <div id="mHorariosWrap">
              <h5 style="font-size:.82rem;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);margin-bottom:.6rem;">
                <i class="bi bi-geo-alt-fill" style="color:var(--orange);"></i> Sedes y horarios
              </h5>
...
function verCurso(nombre) {
  ...
  fetch('curso_detalle.php?nombre=' + encodeURIComponent(nombre))
    .then(r => r.json())
    .then(c => {
...
function renderModal(c) {
  ...
  // SEDES CON HORARIOS Y CUPOS
  const horEl = document.getElementById('mHorarios');
  horEl.innerHTML = '';
  if (c.sedes && c.sedes.length) {
    c.sedes.forEach(sede => {
      const totalDisp = (sede.grupos||[]).reduce((s,g) => s + parseInt(g.disponibles||0), 0);
      const dispColor = totalDisp > 0 ? '#16a34a' : 'var(--orange)';
      const dispTxt   = totalDisp > 0 ? totalDisp + ' cupos disponibles' : 'Sin cupos &mdash; Lista de espera';
      let gruposHtml = '';
      (sede.grupos||[]).forEach((g, idx) => {
        const disp = parseInt(g.disponibles);
        const col  = disp > 3 ? '#16a34a' : (disp > 0 ? '#ca8a04' : '#ef4444');
        const bg   = disp > 3 ? '#f0fdf4' : (disp > 0 ? '#fefce8' : '#fef2f2');
        const mod  = g.modalidad !== 'presencial' ? `<span style="font-size:.62rem;background:rgba(29,169,154,.1);color:#0F6E56;padding:.1rem .4rem;border-radius:6px;">${g.modalidad}</span>` : '';
        gruposHtml += `
          <div style="display:flex;align-items:center;gap:.6rem;padding:.55rem .75rem;background:${bg};border:1px solid ${col}22;border-radius:10px;margin-bottom:.4rem;">
            <div style="width:22px;height:22px;background:${col};color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.65rem;font-weight:800;flex-shrink:0;">${idx+1}</div>
            <div style="flex:1;font-size:.82rem;">
              <strong style="color:#1a2234;">${dias[g.dia_semana]||g.dia_semana}</strong>
              <span style="color:var(--muted);margin-left:.3rem;">${g.hora_inicio.substring(0,5)} &ndash; ${g.hora_fin.substring(0,5)}</span>
              ${mod}
            </div>
            <span style="font-size:.72rem;font-weight:800;color:${col};white-space:nowrap;">${disp>0?disp+' cupos':'Lleno'}</span>
          </div>`;
      });
      horEl.innerHTML += `
        <div style="background:#fff;border:1.5px solid var(--border);border-radius:14px;padding:1rem 1.1rem;margin-bottom:.9rem;">
          <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:.5rem;margin-bottom:.8rem;flex-wrap:wrap;">
            <div>
              <div style="font-size:.92rem;font-weight:800;color:#1a2234;">
                <i class="bi bi-geo-alt-fill" style="color:var(--orange);margin-right:.25rem;"></i>${sede.sede_nombre}
              </div>
              <div style="font-size:.72rem;color:var(--muted);margin-top:.1rem;">${sede.sede_ciudad}</div>
            </div>
            <span style="font-size:.72rem;font-weight:700;color:${dispColor};display:flex;align-items:center;gap:.3rem;">
              <span style="width:7px;height:7px;border-radius:50%;background:${dispColor};display:inline-block;flex-shrink:0;"></span>${dispTxt}
            </span>
          </div>
          ${(sede.grupos||[]).length > 0
            ? `<div style="font-size:.7rem;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.4rem;">Elige tu horario</div>${gruposHtml}`
            : '<p style="font-size:.82rem;color:var(--muted);margin:0;">Sin horarios publicados a&uacute;n.</p>'
          }
          <a href="<?= BASE_URL ?>public/registro.php?curso=${sede.curso_id}"
             style="display:flex;align-items:center;justify-content:center;gap:.4rem;margin-top:.8rem;padding:.6rem;background:${totalDisp>0?'var(--orange)':'#94a3b8'};color:#fff;border-radius:10px;font-weight:800;font-size:.84rem;text-decoration:none;transition:all .2s;"
             onmouseover="this.style.opacity='.88'" onmouseout="this.style.opacity='1'">
            <i class="bi bi-person-plus-fill"></i> ${totalDisp>0?'Inscribirse en '+sede.sede_nombre:'Lista de espera'}
          </a>
        </div>`;
    });
  } else {
  ...
  // INFO EXTRA
  const totalGrupos = (c.sedes||[]).reduce((s,sede) => s + (sede.grupos||[]).length, 0);
  document.getElementById('mInfoExtra').innerHTML = `
    <div style="display:flex;gap:1rem;flex-wrap:wrap;">
      ...
      <div style="text-align:center;"><div style="font-size:1.1rem;font-weight:900;color:#16a34a;font-family:'Poppins',sans-serif;">${(c.sedes||[]).length}</div><div style="font-size:.7rem;color:var(--muted);font-weight:600;">Sedes</div></div>
      <div style="text-align:center;"><div style="font-size:1.1rem;font-weight:900;color:#16a34a;font-family:'Poppins',sans-serif;">${totalGrupos}</div><div style="font-size:.7rem;color:var(--muted);font-weight:600;">Grupos</div></div>
    </div>`;

  // Ocultar btn inscribir global (ahora es por sede)
  document.getElementById('mBtnInscribir').style.display = 'none';
