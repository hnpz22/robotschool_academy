<?php
// modulos/asistencia/eliminar.php
...
if (!$sesion_id) { header('Location: ' . $U . 'modulos/asistencia/index.php'); exit; }
...
if (!$sesion) { header('Location: ' . $U . 'modulos/asistencia/index.php'); exit; }

$pdo->prepare("DELETE FROM sesiones WHERE id=?")->execute([$sesion_id]);

header('Location: ' . $U . 'modulos/asistencia/index.php?grupo=' . $filtro_grupo . '&mes=' . $filtro_mes . '&msg=sesion_eliminada');
exit;
